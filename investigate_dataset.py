"""
author: audreyc
Open a dataset.  Figure out what's in there.  Plot.
"""
...
def show_graphs(data, column_names):
    """
    this shows graphs/histograms of the data
    """
    for show_key in ['datekey', 'expense_key', 'spendcat']:
        if show_key not in column_names:
            continue
        keys = list(data[show_key].keys())
        keys.sort()
        indexes = np.arange(len(keys))
        y_values = [data[show_key][k] for k in keys]
        plt.bar(indexes, y_values, align='center', width=1)
        plt.xticks(indexes, keys, rotation='vertical', ha='center')
